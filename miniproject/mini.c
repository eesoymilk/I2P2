	...
	for (int i = 0; in[i]; i++) {
		if (isspace(in[i])) // ignore space characters
			...
			case '-':
				...
				break;
			case '=':   (*now) = new_token(ASSIGN, 0);  break;
			case '*':   (*now) = new_token(MUL, 0);     break;
			case '/':   (*now) = new_token(DIV, 0);     break;
			case '%':   (*now) = new_token(REM, 0);     break;
			case '(':   (*now) = new_token(LPAR, 0);    break;
			case ')':   (*now) = new_token(RPAR, 0);    break;
			case ';':   (*now) = new_token(END, 0);     break;
		...
		case UNARY_EXPR:
            ...
            switch (arr[l].kind) {
				...
				case MINUS:
                    ...
                    return now;
                    break;
				default: return parse(arr, l, r, POSTFIX_EXPR);
			}
		case POSTFIX_EXPR:
...
void codegen(AST *root)
{
	...
	switch (root->kind) {
		...
		case SUB:
			...
			codegen(root->lhs);
			codegen(root->rhs);
			if (root->kind == ADD)	printf("add ");
			if (root->kind == SUB)	printf("sub ");
			printf("r%d r%d r%d\n", rd, rd, --REG);
			break;
		case MUL:
		case DIV:
		case REM:
			...
			codegen(root->lhs);
			codegen(root->rhs);
			if (root->kind == MUL)	printf("mul ");
			if (root->kind == DIV)	printf("div ");
			if (root->kind == REM)	printf("rem ");
			printf("r%d r%d r%d\n", rd, rd, --REG);
			break;
		case PREINC:
		case PREDEC:
			...
			idx = 4 * (tmp->val - 'x');
			printf("load r%d [%d]\n", REG, idx);
			if (root->kind == PREINC)	printf("add "); 
			if (root->kind == PREDEC)	printf("sub "); 
			printf("r%d r%d 1\n", REG, REG);
			printf("store [%d] r%d\n", idx, REG);
			REG++;
			break;
		...
		case POSTDEC:
			...
			idx = 4 * (tmp->val - 'x');
			printf("load r%d [%d]\n", REG, idx);
			if (root->kind == POSTINC)	printf("add "); 
			if (root->kind == POSTDEC)	printf("sub "); 
			printf("r%d r%d 1\n", REG + 1, REG);
			printf("store [%d] r%d\n", idx, REG + 1);
			REG++;
			break;
